...
|-------------|---------|---------|
| `Agentvibe` | Display name in agent prompts and brand voice | `Acme` |
| `agentvibe` | lower-case slug used in env vars / urls | `acme` |
| `AGENTVIBE` | UPPER prefix for env vars (e.g. `ACME_API_KEY`) | `ACME` |
| `AGENTVIBE-` | Linear / Jira ticket prefix | `ACME-` |
| `agentvibe.com` | Production domain | `acme.com` |
| `https://agentvibe.com` | Marketing site URL (often a separate platform) | `https://acme.com` |
| `https://github.com/Adam077K/agentvibe` | GitHub repo URL | `https://github.com/org/repo` |
...
| `.claude/hooks/*.sh` and `*.js` | Hooks run on every action. Read each one before enabling. `gsa-context-monitor.js` references env var `AGENTVIBE_NO_AUTOCOMPACT` — set it or remove the check. |
