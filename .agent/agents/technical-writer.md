...
2. `CLAUDE.md` — Agentvibe stack, Supabase table names, file path conventions
...
git commit -m "docs(api): add scan/start reference doc (AGENTVIBE--212)"
```
...
```json
{
  ...
  "commits": [
    "docs(api): add scan/start reference doc with rate-limit error codes (AGENTVIBE--212)"
  ],
