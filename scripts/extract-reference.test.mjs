...
test('fitLeading RECOVERS the curve build-tokens implements, from data generated by it', () => {
  // THE POSITIVE CONTROL FOR THE WHOLE FITTER. lineHeight(s) = peak - (|s-peakAt|/peakAt)^exponent * falloff.
  // Generate points from known parameters and require them back. Without this the refusals below
  // would be indistinguishable from a fitter that cannot fit anything.
  const [peak, peakAt, falloff, exponent] = [1.55, 16, 0.62, 1.1];
  const sizes = [10, 12, 14, 16, 18, 20, 24];
  const rows = sizes.map((size) => ({ size, leadingRatio: peak - Math.pow(Math.abs(size - peakAt) / peakAt, exponent) * falloff, count: 50 }));
  const f = fitLeading(rows, { uiSizes: sizes });
  assert.equal(f.peak, peak);
  assert.equal(f.peakAt, peakAt);
  assert.equal(f.falloff, falloff);
  assert.equal(f.exponent, exponent);
  assert.equal(f.residual, 0);
});

test('fitLeading REFUSES the curve when real data does not lie on it, and reports the residual', () => {
  // linear.app, measured 2026-08-29. Its leading is NOT monotone in distance from the peak —
  // 12px sits further below the peak than 10px does, while being closer to it — so no curve of
  // this family passes through the points. The parameters go null and the residual is reported.
  const rows = [[10, 1.4, 26], [11, 1.273, 3], [12, 1.167, 171], [13, 1.5, 143], [14, 1.714, 218], [15, 1.6, 52], [16, 2.75, 4]].map(([size, leadingRatio, count]) => ({ size, leadingRatio, count }));
  const f = fitLeading(rows, { uiSizes: [10, 11, 12, 13, 14, 15, 16] });
  assert.equal(f.peak, 1.714);
  assert.equal(f.peakAt, 14);
  assert.equal(f.falloff, null);
  assert.equal(f.exponent, null);
  assert.ok(f.residual > 0.1, `residual ${f.residual} should exceed the 0.1 ceiling`);
  assert.ok(f.notes.some((n) => /does not fit/.test(n) && /residual/.test(n)), 'the refusal must carry the number it was made on');
});

test('the fit always declares that the FUNCTION is a fit and only the SHAPE is sourced', () => {
  const sizes = [10, 12, 14, 16, 18, 20, 24];
  const rows = sizes.map((size) => ({ size, leadingRatio: 1.55 - Math.pow(Math.abs(size - 16) / 16, 1.1) * 0.62, count: 50 }));
  const f = fitLeading(rows, { uiSizes: sizes });
  assert.ok(f.notes.some((n) => /the exact FUNCTION is a fit, not a standard/.test(n)),
    'a consumer must not read a fitted exponent as a standard; a poor fit is evidence about the formula');
});

test('too few falloff points leaves peak fitted and the curve null — not the other way round', () => {
  // peak/peakAt are measurable maxima and survive; exponent/falloff need three points off the peak.
  const f = fitLeading([{ size: 12, leadingRatio: 1.4, count: 9 }, { size: 14, leadingRatio: 1.5, count: 9 }, { size: 16, leadingRatio: 1.45, count: 9 }], { uiSizes: [12, 14, 16] });
  assert.equal(f.peak, 1.5);
  assert.equal(f.peakAt, 14);
  assert.equal(f.falloff, null);
  assert.equal(f.exponent, null);
  assert.ok(f.notes.some((n) => /need 3 and are null rather than guessed/.test(n)));
  assert.equal(fitLeading([{ size: 12, leadingRatio: 1.4 }], {}).peak, null);
});

test('NOT ONE of the five measured references fits the curve — reported, not smoothed away', () => {
  // The finding this harness exists to surface, pointed at our own schema rather than at a rule.
  // If a later change makes any reference fit, this test goes red and that is the right failure:
  // it means either the formula changed or the corpus did, and both deserve a decision.
  const unfitted = [];
  for (const [slug, sizes] of Object.entries(FIX)) {
    const s = deriveSeeds({ type: { sizes } });
    if (s.type.leading.falloff === null) unfitted.push(slug);
  }
  assert.deepEqual(unfitted.sort(), Object.keys(FIX).sort(),
    'every reference should currently fail the curve fit; a reference that now fits is a real change, not a flake');
});
