...
```bash
pnpm -F @agentvibe/web dev &   # start dev server if not running
```
...
git commit -m "design(scan): implement scan results screen with tier score cards (AGENTVIBE--N)"
```
...
```json
{
  ...
  "commits": [
    "design(scan): implement scan results page with per-engine score cards (AGENTVIBE--118)",
    "design(scan): add responsive mobile layout for ScanResultCard (AGENTVIBE--118)"
  ],
  "screenshots": [
