...
### Step 2 — Framing (if feature is vague)
If the request is an idea without a spec, `orchestrator` dispatches `framer` to write a PRD first:
- Problem validation
- RICE scoring
- PRD with acceptance criteria written to `docs/04-product/specs/[name].md`

### Step 3 — Technical Breakdown
`orchestrator` + `builder` explore the codebase and produce:
...
- `orchestrator` does the codebase exploration and hands off to `builder` for the actual breakdown
