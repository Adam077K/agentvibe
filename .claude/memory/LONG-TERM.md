...
- **A lane that has gone idle can still wake and act on a queued message** (2026-09-07): a correction arrived after `research-fold` committed, so it resumed and became a second live writer on a file another lane was mid-read of. Before dispatching a replacement, check whether the original has woken — and note that `sourcer` has no `Write` or `Bash`, so dispatch `reviewer` when the deliverable is a file.
