...
Deploy, CI, jobs, payments, email, analytics — running it in production
...
| `inngest` | Inngest expert for serverless-first background jobs, event-driven workflows, and durable execution without managing queues or workers. Use when:… |
...
| `paddle-integration` | Agentvibe billing patterns using Paddle: webhook signature verification, checkout session creation, customer portal URL generation, price ID… |
| `payment-integration` | Integrate Stripe, PayPal, and payment processors. Handles checkout flows, subscriptions, webhooks, and PCI compliance. Use PROACTIVELY when… |
...
| `stripe-integration` | Implement Stripe payment processing for robust, PCI-compliant payment flows including checkout, subscriptions, and webhooks. Use when integrating… |
...
| `worktree-isolation-pattern` | The exact git worktree create, detect, and clean pattern for Agentvibe workers: detect-or-create from main-repo-root, child worktree commands,… |
