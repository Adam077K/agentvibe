...
| **#47** | **Three more tests whose verdict depends on the machine, in files nobody has examined.** Surfaced while building #36: `stream.test.ts`'s 40 s SSE reaper and two in `views.test.tsx` failed under ~200% desktop CPU load, one taking **964 s against a 5 s timeout**. Each passes in isolation and none imports the code under change, so they are load-sensitive rather than broken — which is the same class as #43/#43a/#43b and the reason #45 exists. Nobody has audited these two files for the property. **The cheap first pass is to find every test that asserts on a duration and ask what it does under load**, since the failure is silent until it is a red build on an unrelated PR |
