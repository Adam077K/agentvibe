...
| 6 | **Reader, budgets, stall detection, routines** — *also owns the memory-file collapse* — ✅ **DONE** | **Reader:** it is `ledger sweep`, not an agent — six deterministic queries and its own anti-patterns forbade the one judgement in scope, so the file was deleted and the roster is **6 engines**. **The repo's first clock:** measured beforehand — no crontab, no launchd, no Actions `schedule:`, no Inngest config, while 12 routines declared cron strings. **Budgets + stall:** ceiling on the rolling 5-hour window (2M warn / 3M block output tokens, against a measured peak of 1,961,285), stall at 200k/400k since the last durable artifact; measurement went 9,023ms → 13ms by using append-only. **Routines:** war-room deleted entirely — 25 files / 3,256 lines in three populations, all with existing homes; all three `harness-health` routines turned out to be scripts. **Memory:** only `CODEBASE-MAP.md` is generated, non-lossy provably (the pre-migration file was empty); `DECISIONS.md` cannot be a view because the schema has no field for a rejected option. **Three gate criteria amended, each with the measurement that forced it.** ~~§0 closed: lenses and playbooks are injected at session start~~ — **CORRECTED 2026-08-12, and this was wrong when written.** The hook emits 25,613 bytes correctly, but the runtime inlines only a ~2KB preview and persists the rest to a file path, so what reaches agent context is a pointer, not the payload — discretionary, which is the exact opposite of the property claimed. It went unnoticed because `c-lenses-and-playbooks-are-loaded` was verified by a command testing **what the hook emits**, never what the session receives (standing rule 3). The claim now checks payload size and fails until the fix lands; the fix is a router — lens ids plus one-line summaries, ~1.5KB — the same cure Phase 7 found for skills |
| 7 | **Skills** *(parallelizable with 3–6)* — ✅ **DONE** | **147 → 84 by the four tests, then rebuilt to 134** as the cuts were audited and better sources found. Routers in place: discovery went from **~15,000 tokens to ~1,070** (`routers/INDEX.md` → one of 7 namespaces → the skill), and the routers live outside the skill directories so they cannot inflate the manifest they exist to avoid. Every cut names the test it failed in [CURATION.yml](../../.claude/skills/CURATION.yml); `check:curation` fails on drift, and refuses a cut whose named survivor is absent — **that check fired six times the moment it was written**. **Nine of 37 `near_duplicate` cuts were proven wrong**, all by the same error: judged from names rather than from reading both files. Six of those folded into a survivor that was itself cut, so the content vanished rather than moved — which is why the documentation category emptied. Not done, and named: the 803 external candidates across 11 corpora were never evaluated; this is the local library honestly cut, plus targeted additions with provenance |
| 8 | **Mission Control** — 🔨 **IN PROGRESS**, split into **8a** (read plane) and **8b** (Dispatch). See [PHASE-8A-STATUS.md](PHASE-8A-STATUS.md) | ~~Dispatch a goal into a second project…; claims land in that repo's ledger~~ — **this gate was unreachable as written and is amended.** Measured 2026-08-12: **no sibling project has a ledger** (zero `scripts/ledger.mjs`, zero `CLAIM-LEDGER.md` across all 13), so no claim could land in one. Reaching it requires installing the spine elsewhere first — propagation, i.e. **Phase 9**, which the 2026-08-11 founder decision forbids before Phase 9. Phase 8's gate depended on Phase 9. Six of the seven views only read and need no spine in the target; **Dispatch is the only view that writes**, so the phase splits there. **8a gate:** every displayed figure reproducible by an independent command · a mutated fixture turns a test red · live data from ≥3 non-`agentvibe` projects · cold start < 3 s, refresh < 250 ms · `npm run check` exit 0 from a clean clone after `bun install`. **8b gate:** the original, once Phase 9 gives it targets that run the current harness. **PR1 of 5 merged** ([#21](https://github.com/Adam077K/agentvibe/pull/21)) — the rail, and this repo's first dependency |
