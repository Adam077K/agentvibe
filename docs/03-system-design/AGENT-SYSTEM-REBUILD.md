...
| Field | Value |
|---|---|
| **Status** | Accepted · **Phases 1, 2, 3, 4a and 5 complete** · **Phase 4b next** (roster collapse — the irreversible half). Phase 5 was brought forward ahead of 4b; see the phase table for why |
| **Date** | 2026-08-11 |
| **Owner** | ceo |
...
| 4 | **Roster collapse** — 7 engines + lenses — **4a ✅ DONE, 4b next** | **4a (additive):** 8 domain + 8 review lenses, content-linted; single-model "independent" panel fails the lint → **✅**; all 52 decorative `mcpServers` deleted and the field now fails unless configured; 4 read-only reviewers stripped of `Write`. **4b (subtractive, not started):** 7 engine files, old-name routing, delete the 19 superseded agents. Gate outstanding: *read-only engine cannot write, verified by attempt* — needs one probe the founder runs, since subagent spawning is disabled in these sessions |
...
| 6 | **Reader, budgets, stall detection, routines** — *also owns the memory-file collapse* | Stall escalates instead of looping; ceiling fires *before* dispatch with a named reason; stale reader stamp warns at session start; **`DECISIONS.md` / `LONG-TERM.md` / `USER-INSIGHTS.md` / `CODEBASE-MAP.md` are generated views over the ledger, with the migration proven non-lossy against the pre-migration files** |
