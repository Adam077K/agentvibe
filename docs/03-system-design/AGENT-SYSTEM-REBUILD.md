...
| 6 | **Reader, budgets, stall detection, routines** — *also owns the memory-file collapse* — 🔄 **IN PROGRESS** | **Reader: ✅ done.** It is `ledger sweep`, not an agent — all six fields of its return contract were deterministic queries and its own anti-patterns forbade the one judgement in scope, so `.claude/agents/reader.md` was deleted and the roster is **6 engines**. Run by [ledger-sweep.yml](../../.github/workflows/ledger-sweep.yml) (the repo's first real clock — measured: no crontab, no launchd, no Actions `schedule:`, no Inngest config existed, while 12 routines declared cron strings) and by [session-start.js](../../.claude/hooks/session-start.js), which also injects the lens and playbook files — **the §0 fix: both were linted on every PR and loaded by nothing.** Three gate criteria amended, each with the measurement that forced it: the budget guards *every tool call* not dispatch (0 of 414M session tokens went through `Task`), the ceiling is per rolling 5-hour window (peak measured 1.96M output tokens), and `DECISIONS.md` does not become a generated view (the ledger has no field for rejected options, so "non-lossy" fails by construction). Remaining: budgets, stall detection, war-room collapse, memory |
