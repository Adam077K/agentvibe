...
- `scripts/warroom-engine.test.mjs` created with 14 tests (wired into `test:warroom`); later sessions grew it to 69 `test(` cases by 2026-09-10, so derive the count — `grep -c '^test(' scripts/warroom-engine.test.mjs` — rather than reading it here. Ten mutations applied to my own fix, each confirmed RED; one mutation silently failed to apply on the first attempt and would have left a test unproven.
