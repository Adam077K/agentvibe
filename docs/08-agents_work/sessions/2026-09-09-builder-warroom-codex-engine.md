...
- Corrected a stale premise in `PROFILES.codex` — Codex IS installed now — while keeping `verified_against_binary: false`, because the flag governs the parse of a COMPLETED turn and every probe stopped at 401. Real finding recorded: `codex exec` never exits on auth failure, so a resolver waiting on it hangs instead of returning `unresolved`.
